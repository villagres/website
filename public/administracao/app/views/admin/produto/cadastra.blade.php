		...
		<div class="col-md-9">
			...
			<form action="{{Route('admin.gravaProduto')}}" method="POST" enctype="multipart/form-data">
						...
						<select name="cor" id="cor">
							...
							@if(isset($cores))
								@foreach($cores as $cor)
									<option value="{{$cor->id}}">{{$cor->filtro}}</option>
								@endforeach
							@endif
						</select>
					</div>
						...
						<select name="indicacao" id="indicacao">
							...
							@if(isset($indicacoes))
								@foreach($indicacoes as $indicacao)
									<option value="{{$indicacao->id}}">{{$indicacao->filtro}}</option>
								@endforeach
							@endif
						</select>
					</div>
						...
						<select name="layout" id="layout">
							...
							@if(isset($layouts))
								@foreach($layouts as $layout)
									<option value="{{$layout->id}}">{{$layout->filtro}}</option>
								@endforeach
							@endif
						</select>
					</div>
				...
				<div class="row">
					...
					<div class="col-md-1"><input type="checkbox" id="rodape" name="rodape"></div>
				</div>
				<div class="row">
					<div class="col-md-2"><span>Lançamento:</span></div>
					<div class="col-md-1"><input type="checkbox" id="lancamento" name="lancamento"></div>
				</div>
				<div class="row">
